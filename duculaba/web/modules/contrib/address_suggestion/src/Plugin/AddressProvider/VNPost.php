...
/**
 * Defines a Vietnam post plugin for address_autocomplete.
 *
...
class VNPost extends AddressProviderBase {
  ...
  public function processQuery($string, $settings) {
    ...
    $url = !empty($settings['endpoint']) ? $settings['endpoint'] : 'https://maps.vnpost.vn/api/autocomplete';
    ...
    if (!empty($content['code']) && $content['code'] == 'OK') {
      foreach ($content['data']["features"] as $key => $feature) {
        ...
        $results[$key]['label'] = $properties["label"];
